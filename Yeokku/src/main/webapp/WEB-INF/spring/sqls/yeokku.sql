...
CREATE SEQUENCE USERBOARDSEQ NOCACHE;
...
ALTER TABLE USERBOARD DROP COLUMN USER_DETAILADDRESS;
...
CREATE TABLE REPORTBOARD(
  ...
  REPORT_CONFIRM VARCHAR2(4) DEFAULT 'N' CHECK(REPORT_CONFIRM IN ('Y','B','N'))
);


/* 여행지 테이블 */
CREATE TABLE TOURBOARD(
  TOUR_CONTENTID NUMBER NOT NULL,
  TOUR_CONTENTTYPEID NUMBER NOT NULL,
  TOUR_VIEW NUMBER,
  PRIMARY KEY(TOUR_CONTENTID,TOUR_CONTENTTYPEID)
);

/* 여행지 좋아요(LIKE TOUR) 테이블 */
CREATE TABLE LTBOARD(
  LT_CONTENTID NUMBER NOT NULL,
  LT_USERNO NUMBER NOT NULL,
  LT_TITLE VARCHAR2(500),
  LT_FIRSTIMAGE VARCHAR2(1000),
  PRIMARY KEY(LT_CONTENTID,LT_USERNO),
  CONSTRAINT fk_tour_like_user FOREIGN KEY (LT_USERNO) REFERENCES USERBOARD(USER_NO) ON DELETE CASCADE
);
SELECT * FROM LTBOARD;
DELETE FROM LTBOARD WHERE LT_USERNO = 1;
COMMIT;
...
/*추가*/
CREATE SEQUENCE TRBOARDSEQ NOCACHE;
INSERT INTO TRBOARD VALUES(TRBOARDSEQ.NEXTVAL, 100, 100, 'HELLO', TO_CHAR(SYSDATE,'YYYY-MM-DD'));

DELETE FROM TRBOARD;
COMMIT;
SELECT * FROM TRBOARD;

SELECT * FROM CATBOARD;
commit;
...
CREATE TABLE TCBOARD(
 ...
 TC_PRE_CONTENTID NUMBER,
 TC_NEXT_CONTENTID NUMBER,
 TC_CONTENTID NUMBER,
 TC_CONTENTTYPEID NUMBER,
 TC_DATE DATE,
 TC_VIEW NUMBER,
...
DROP TABLE LTCBOARD;
CREATE TABLE LTCBOARD(
  LTC_TCNO NUMBER NOT NULL,
  LTC_USERNO NUMBER NOT NULL,
  LTC_DATE DATE
);
COMMIT;
...
CREATE TABLE TCPBOARD(
  ...
  TCP_CONTENTID NUMBER NOT NULL,
  TCP_CONTENTTYPEID NUMBER,
...
ALTER TABLE TCBOARD ADD TC_CONTENT LONG NOT NULL;
